...
enemies = [
    {"name": "Goblin", "health": 30, "damage": 5, "gold": 10},
    {"name": "Skeleton", "health": 45, "damage": 7, "gold": 15},
    {"name": "Orc", "health": 80, "damage": 12, "gold": 25},
    {"name": "Dark Mage", "health": 60, "damage": 15, "gold": 30},
    {"name": "Dragon", "health": 200, "damage": 25, "gold": 100}
]
...
3. 🚶‍♂️ Return to town """)
        ...
        if choice == "1" or "health" in choice:
            ...
            slow_print("\n🚶‍♂️ You leave the shop and return to town.")
            return
        else:
...
def buy_item(item_name):
    ...
    ''' if user already bought the item don't let them to buy again.But they can buy the health potion even if
        they have it already'''
    if item_name in player["items"] and item_name != "health potion":
        slow_print(f"\n⚔️ You already have a {item_name}")
        return
...
def forest():
    ...
    while True:
        display_stats()
        print("""\nWhat would you like to do?
1. 🔍 Explore deeper (chance to find teasure or enemies)
2. 🏠 Return to town
3. 🏕️ Set up camp (restore 10 health)""")
        ...
        if choice == "1" or "explore" in choice:
            ...
            print("\n❓I don't understand that option! please try again.")
...
def explore():
    ...
    # Random encounter: 60% chance on enemy, 30% chance on teasure and 10% on nothing
    encounter = random.choices(
        ["enemy", "teasure", "nothing"], [60, 30, 10])[0]

    if encounter == "enemy":
        enemy_encounter()
    elif encounter == "teasure":
        treasure_encounter()
    else:
        slow_print("\n🤷‍♂️ You explore for a while but find nothing interesting.")
    ...
    while enemy_health > 0 and player["health"] > 0:
        ...
        if choice == "1" or "attack" in choice:
            ...
            if "health potion" in player["items"]:
                ...
                slow_print(
                    f"\n🧪 You used a health potion and restored {items['health potion']['health']} health!")
            else:
            ...
            if random.random() > 0.5:
                slow_print("🏃 You managed to espace!")
                return
            else:
                slow_print("😨 You couldn't espace!")
                player['health'] -= enemy['damage']
                slow_print(
                    f"😱 The {enemy['name']} attacks you for {enemy['damage']} damage!")
...
def treasure_encounter():
    ...
    # this will generate a num b/w 0.0 - 0.99
    if random.random() < 0.2 and "health potion" not in player["items"]:
        ...
        slow_print("\n💰 You found a small pounch with some gold inside!")
        slow_print(f"✨ You gained {gold_found} gold!")


def rest():
    if player["health"] == 100:
        slow_print("\n😴 You're already at full health. No need to rest.")
        return
    if player["gold"] >= 10:
        player["gold"] -= 10
        player["health"] = 100
